...
```bash
npm test     # 9 тестов: контрольный комплект организатора, загрузка комплекта файлов, авторизация и роли, помощь и заключение
```
...
Скопируйте `.env.example` в `.env` и укажите `OPENAI_API_KEY` — ключ API с platform.openai.com, начинается с `sk-` (ключ другого вида сервер игнорирует и работает локально). После этого в интерфейсе становится доступен переключатель «AI-проверка выводов». Модель получает только кандидатов и цитаты (`store: false`) и отвечает по строгой JSON-схеме. Если ключа нет или произошла ошибка, сохраняется локальный результат. **Основной сценарий работает полностью без ключа.**
