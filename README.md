...
| Ограничение: без неподтверждённых утверждений | Детерминированное ядро не может сослаться на несуществующий пункт. LLM проверяет только готовых кандидатов и не добавляет новые факты ([AI-контракт](docs/AI_CONTRACT.md)) |
...
```bash
npm test     # 6 тестов, включая контрольный комплект организатора
```
...
6. Подтвердить один вывод, отклонить другой с комментарием, показать прогресс проверки.
...
| Слой | Стек | Почему |
|---|---|---|
| Сервер | Node.js 22+, Express 5, Helmet, rate-limit | Один язык с фронтендом и мобильным приложением, быстрый старт |
...
| AI | OpenAI Responses API + Structured Outputs (опционально) | Второе мнение по кандидатам без права добавлять факты |
...
| POST | `/api/analyses` | multipart: `before`, `after`, `name`, `useAi` |
| GET | `/api/analyses` | История анализов |
| GET | `/api/analyses/:id` | Полный результат: `findings`, `units`, `functionMap`, `summary` |
| PATCH | `/api/analyses/:id/findings/:findingId` | Решение эксперта: `{status, comment}` |
| GET | `/api/analyses/:id/report` | Печатное заключение (HTML) |

## AI (опционально)

Скопируйте `.env.example` в `.env` и укажите `OPENAI_API_KEY`. После этого в интерфейсе становится доступен переключатель «AI-проверка выводов». Модель получает только кандидатов и цитаты (`store: false`) и отвечает по строгой JSON-схеме. Если ключа нет или произошла ошибка, сохраняется локальный результат. **Основной сценарий работает полностью без ключа.**
...
- Нет авторизации и ролей. Для промышленного контура нужны SSO, TLS и антивирусная проверка, см. [SECURITY.md](docs/SECURITY.md).
...
Предыдущий прототип команды для кейса AI Sana (TaskForge AI) сохранён в [`archive/taskforge-ai/`](archive/taskforge-ai/).
