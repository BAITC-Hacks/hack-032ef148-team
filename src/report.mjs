...
export function renderReport(record) {
  ...
  const redistribution = record.findings.filter((finding) => ["function_lost", "function_duplicate", "conflict_risk"].includes(finding.type))
    .map((finding) => `<tr><td>${escapeHtml(labels[finding.type])}</td><td><b>п. ${escapeHtml(finding.evidence[0]?.clause)}</b> · ${escapeHtml(finding.evidence[0]?.unit)}<br>${escapeHtml(finding.evidence[0]?.snippet)}</td><td>${finding.suggestedOwner ? `<b>${escapeHtml(finding.suggestedOwner)}</b><br>` : ""}${escapeHtml(finding.recommendation)}</td></tr>`).join("");
  ...
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escapeHtml(record.name)} — заключение БАТЫС AI</title><style>
    ...
    <h2>Рекомендации по перераспределению функций</h2>
    ${redistribution ? `<table><thead><tr><th>Проблема</th><th>Функция</th><th>Рекомендация</th></tr></thead><tbody>${redistribution}</tbody></table>` : "<p>Потерь и пересечений функций не обнаружено.</p>"}
